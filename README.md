...
**Dataset:** [https://huggingface.co/datasets/rfsit/MATCH-A](https://huggingface.co/datasets/rfsit/MATCH-A)  
**Code:** [https://github.com/raphaelfrick/MATCHA](https://github.com/raphaelfrick/MATCHA)
...
**Columns:**
- `query_path`: Path to the manipulated query image (relative to dataset root)
- `positive_path`: Path to the authentic source image (empty string for orphans)
...
### Gallery Dataset (`reference_db/`)

Contains authentic gallery images that serve as the trusted corpus for matching.

**Columns:**
- `image_path`: Path to the authentic gallery image (relative to dataset root)
- `split`: Dataset split ('train', 'val', or 'test')
...
queries_ds = load_dataset("rfsit/MATCH-A", data_dir="queries", split="train")

# Load gallery dataset
gallery_ds = load_dataset("rfsit/MATCH-A", data_dir="gallery", split="train")

# Access a sample
sample = queries_ds[0]
print(sample["query_path"])  # Path to query image
print(sample["positive_path"])  # Path to positive image (empty if orphan)
...
| **Distortion** | lens distortion, chromatic aberration, fisheye |
...
- **$FPR_{orph}(\tau)$**: Fraction of orphans for which any candidate is returned. Lower is safer.
...
- **$TNR_{orph}(\tau)$**: Fraction of orphans correctly yielding no candidate.
...
| FPR_orph(τ) | 1.0000 | **0.9815** | 0.9940 |
...
- **License**: Images governed by [FLUX.1 [dev] Non-Commercial License](https://github.com/black-forest-labs/flux/blob/main/model_licenses/LICENSE-FLUX1-dev)
...
If you would like to contribute, feel free to leave a comment in the community tab or open an issue or pull request on our GitHub repository.
